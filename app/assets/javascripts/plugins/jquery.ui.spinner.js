...
function isVisible(element) {
	...
	(visible && jQuery.each(jQuery.dir(element, 'parentNode'), function() {
			...
			? this.each(function(i, el) {
				(isFF2
					? el.setAttributeNS("http://www.w3.org/2005/07/aaa",
						"aaa:" + state, value)
					: jQuery(el).attr("aria-" + state, value));
			})
...
jQuery.widget.prototype = {
	...
	_trigger: function(type, e, data) {
		...
		e = e  || jQuery.event.fix({ type: eventName, target: this.element[0] });
...
jQuery.ui = {
	...
	hasScroll: function(e, a) {
		...
		if (jQuery(e).css('overflow') == 'hidden') { return false; }
...
jQuery.widget('ui.spinner', {
		...
		.parent()
			...
			.find('.ui-spinner-up')
				.bind('mousedown', function(e) {
					jQuery(this).addClass('ui-spinner-pressed');
					if (!self.counter) {
						self.counter = 1;
					}
				...
				.bind('mouseup', function(e) {
					jQuery(this).removeClass('ui-spinner-pressed');
					if (self.counter == 1) {
						self._up(e);
					}
					self._mouseup(e);
	...
	_mousewheel: function(e, delta) {
		var self = this;
		delta = (jQuery.browser.opera ? -delta / Math.abs(delta) : delta);
	...
	_setValue: function(newVal) {
		...
		this.element.val(
			this.options.currency ? 
				jQuery.ui.spinner.format.currency(newVal, this.options.currency) : 
				jQuery.ui.spinner.format.number(newVal, this._decimals)
		);
	},
	_animate: function(d) {
	...
	_propagate: function(n,e) {
		jQuery.ui.plugin.call(this, n, [e, this.ui()]);
		return this.element.triggerHandler(n == 'spin' ? n : 'spin'+n, [e, this.ui()], this.options[n]);
	},
	destroy: function() {
		if (!jQuery.data(this.element[0], 'spinner')) {
			return;
		}
		if (jQuery.fn.mousewheel) {
...
jQuery.extend(jQuery.ui.spinner, {
	defaults: {
		decimals: 0,
		stepping: 1,
		start: 0,
	...
	format: {
		...
		number: function(num, dec) {
			var regex = /(\d+)(\d{3})/;
			for (num = isNaN(num) ? 0 : parseFloat(num,10).toFixed(dec); regex.test(num); num=num.replace(regex, 'jQuery1,jQuery2'));
			return num;
		}
	}
});
